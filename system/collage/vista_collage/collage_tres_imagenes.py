...
class TresImagenesCollage:
    ...
    def evento_elegir_archivo(self, values):
        """ "Esta funcion va guardando en una lista las imagenes
        seleccionadas por el usuario para a futuro formar un collage"""

        filename = values["-ELEGIR ARCHIVO-"]
        self.imagenes.append(filename)
        if len(self.imagenes) == 3:
    ...
    def insertar_img_segun_formato(self):
        ...
        if self.formato == "-FORMATOCUATRO-":
            self.generar_collage_formato_cuatro()
        if self.formato == "-FORMATOSEIS-":
            self.generar_collage_formato_seis()
        if self.formato == "-FORMATOSIETE-":
            self.generar_collage_formato_siete()
    ...
    def generar_collage_formato_cuatro(self):
        ...
        for i in range(len(self.imagenes)):
            ...
            img = img.resize((coords[i][2] - coords[i][0], coords[i][3] - coords[i][1]))
            self.collage.paste(img, coords[i][:2])
        self.imagen_bytes = io.BytesIO()
        self.collage.save(self.imagen_bytes, format="PNG")
        self.window["-PREVIEW-"].update(data=self.imagen_bytes.getvalue())

    def generar_collage_formato_seis(self):
        """ "Esta funcion itera sobre la lista de imagenes para
        ir pegandolas en el collage y luego las imagenes PNG las
        transforma en Bytes
        Divide la pantalla en en tres imagenes,
        una imagen en el margen superior
        y la otras dos imagenes: una en el margen inferior izquierdo
        la otra en el margen inferior derecho"""
        ...
        coords = [
            ...
            (0, upper_height, width // 2, height),
            (width // 2, upper_height, width, height),
        ]
        for i in range(len(self.imagenes)):
            img = Image.open(self.imagenes[i])
            img = img.resize((coords[i][2] - coords[i][0], coords[i][3] - coords[i][1]))
    ...
    def generar_collage_formato_siete(self):
        """ "Esta funcion itera sobre la lista de imagenes para
        ir pegandolas en el collage y luego las imagenes PNG las
        transforma en Bytes
        Divide la pantalla en en tres imagenes,
        una imagen en el margen inferior
        y la otras dos imagenes: una en el margen superior izquierdo
        la otra en el margen superior derecho"""
        ...
        lower_height = height - upper_height
        coords = [
            (0, 0, width // 2, upper_height),
            (width // 2, 0, width, upper_height),
            (0, upper_height, width, height),
        ]
        for i in range(len(self.imagenes)):
            img = Image.open(self.imagenes[i])
            img = img.resize((coords[i][2] - coords[i][0], coords[i][3] - coords[i][1]))
