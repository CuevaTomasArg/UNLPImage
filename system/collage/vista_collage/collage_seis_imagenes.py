...
class SeisImagenesCollage:
    def __init__(self, numero_formato):
        ...
        columna_izquierda = [
            ...
            [
                sg.Input(
                    key="-ELEGIR ARCHIVO 6-",
                    enable_events=True,
                    visible=False,
                    default_text=ruta_imagenes_collage,
                ),
                sg.FileBrowse(
                    "Seleccionar imagen 6",
                    tooltip="Extensión valida .png",
                    target="-ELEGIR ARCHIVO 6-",
                    file_types=tipo_de_archivo,
                    initial_folder=ruta_imagenes_collage,
                ),
            ],
    ...
    def insertar_img_segun_formato(self):
        ...
        if self.formato == "-FORMATOCINCO-":
            self.generar_collage_formato_cinco()

    def evento_elegir_archivo(self, num):
        """Esta función guarda la imagen seleccionada
        por el usuario en una lista, según su posición.
        También actualiza el botón "Guardar" y habilita el campo "Título"
    ...
    def generar_collage_formato_cinco(self):
        """ "Esta funcion itera sobre la lista de imagenes para
        ir pegandolas en el collage y luego las imagenes PNG las
        transforma en Bytes
        Divide la pantalla en en seis,
        tres imagenes en el lado derecho
        y las otras tres en el lado izquierdo"""

        width, height = 600, 400
        self.collage = Image.new("RGB", (width, height))
        coords_left = [
            (0, 0, width // 2, height // 3),
            (0, height // 3, width // 2, 2 * (height // 3)),
            (0, 2 * (height // 3), width // 2, height),
        ]
        coords_right = [
            (width // 2, 0, width, height // 3),
            (width // 2, height // 3, width, 2 * (height // 3)),
            (width // 2, 2 * (height // 3), width, height),
        ]
        for i in range(len(self.imagenes)):
            img = Image.open(self.imagenes[i])
            if i < 3:
                coords = coords_left[i]
            else:
                coords = coords_right[i - 3]
            img = img.resize((coords[2] - coords[0], coords[3] - coords[1]))
            self.collage.paste(img, coords[:2])
        self.imagen_bytes = io.BytesIO()
    ...
    def texto_sobre_collage(self):
        """ "Esta funcion muestra en una segunda ventana como
        queda el collage y guarda en el archivo json las imagenes
